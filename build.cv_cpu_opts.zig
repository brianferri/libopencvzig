...
fn genSimdDeclarations(
    ...
    const joined_blocks = try std.mem.join(b.allocator, "\n", dispatch_blocks.items);
    const joined_modes = try std.mem.join(b.allocator, ", ", dispatch_modes.items);
    ...
    _ = awf.add(b.fmt("{s}.simd_declarations.hpp", .{simd}), b.fmt(
        ...
        \\
    , .{ src_dir, simd, joined_blocks, joined_modes }));
