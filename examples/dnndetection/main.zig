...
const Mat = cv.core.Mat;
const Size = cv.core.Size;
const c_api = cv.c;
...
pub fn main() anyerror!void {
    ...
    var net = cv.dnn.Net.readNet(model_path, config_path) catch |err| {
    ...
    const ratio: f64 = 1;
    const mean = cv.core.Scalar.init(104, 177, 123, 0);
    ...
    while (true) {
        ...
        var blob = try cv.dnn.Blob.initFromImage(
            img,
            ratio,
            Size.init(300, 300),
            mean,
...
fn performDetection(frame: *Mat, results: Mat) void {
    const green = cv.core.Color{ .g = 255 };
    var i: usize = 0;
    while (i < results.total()) {
        ...
        if (confidence > 0.5) {
            ...
            _ = cv.imgproc.rectangle(frame, cv.core.Rect{ .x = left, .y = top, .width = right, .height = bottom }, green, 2);
        }
        i += 7;
    }
}
