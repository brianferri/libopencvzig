...
pub fn drawKeyPoints(src: Mat, kp: []KeyPoint, dst: *Mat, color_: Color, flags: DrawMatchesFlag, allocator: std.mem.Allocator) !void {
    ...
    const c_keypoints = c.KeyPoints{
        .length = @intCast(kp.len),
        .keypoints = @ptrCast(c_keypoints_array.items),
    };
        ...
        for (ret) |dmatches| {
            ...
            var dmatch_list = try std.ArrayList(Self).initCapacity(allocator, len);
            {
                ...
                while (i < len) : (i += 1) {
                    ...
                    try dmatch_list.append(allocator, Self.init(
                        dmatch.queryIdx,
                        dmatch.trainIdx,
                        dmatch.imgIdx,
                        dmatch.distance,
...
const MultiDMatches = struct {
    ...
    pub fn deinit(self: *MultiDMatches, gpa: std.mem.Allocator) void {
        for (self.array.items) |*m| m.deinit(gpa);
...
test "feature2d AgastFeatureDetector" {
    ...
    var kp = try ad.detect(img, testing.allocator);
    defer kp.deinit(testing.allocator);
...
test "feature2d BRISK" {
    ...
    var kp = try br.detect(img, testing.allocator);
    ...
    var kp2 = try br.detectAndCompute(img, mask, &desc, testing.allocator);
    defer kp2.deinit(testing.allocator);
...
test "feature2d FastFeatureDetector" {
    ...
    var kp = try fd.detect(img, testing.allocator);
    defer kp.deinit(testing.allocator);
...
test "feature2d GFTTDetector" {
    ...
    var kp = try gft.detect(img, testing.allocator);
    defer kp.deinit(testing.allocator);
...
test "feature2d KAZE" {
    ...
    var kp = try kaze.detect(img, testing.allocator);
    defer kp.deinit(testing.allocator);
    ...
    var kp2 = try kaze.detectAndCompute(img, mask, &desc, testing.allocator);
    defer kp2.deinit(testing.allocator);
...
test "feature2d MSER" {
    ...
    var kp = try mser.detect(img, testing.allocator);
    defer kp.deinit(testing.allocator);
...
test "feature2d ORB" {
    ...
    var kp = try orb.detect(img, testing.allocator);
    defer kp.deinit(testing.allocator);
    ...
    var kp2 = try orb.detectAndCompute(img, mask, &desc, testing.allocator);
    defer kp2.deinit(testing.allocator);
...
test "feature2d SimpleBlobDetector" {
    ...
    var kp = try sbd.detect(img, testing.allocator);
    defer kp.deinit(testing.allocator);
...
test "feature2d BFMatcher" {
    ...
    const k = 2;
    var matches = try bfm.knnMatch(desc1, desc2, k, testing.allocator);
...
test "feature2d FlannBasedMatcher" {
    ...
    var matches = try fbm.knnMatch(desc1, desc2, k, testing.allocator);
    defer matches.deinit(testing.allocator);
...
test "feature2d DrawMatches" {
    ...
    var matches = try bf.knnMatch(desc1, desc2, 2, testing.allocator);
    defer matches.deinit(testing.allocator);
