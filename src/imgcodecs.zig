...
pub fn imEncode(file_ext: FileExt, img: Mat, allocator: std.mem.Allocator) !std.ArrayList(u8) {
    ...
    _ = c.Image_IMEncode(@ptrCast(file_ext.toString()), img.ptr, cvp);
...
pub fn imEncodeWithParams(file_ext: FileExt, img: Mat, comptime params: []const IMWriteParam, allocator: std.mem.Allocator) !std.ArrayList(u8) {
    ...
    STDVector.init(cvp);
    defer STDVector.deinit(cvp);
    _ = c.Image_IMEncode_WithParams(@ptrCast(file_ext.toString()), img.ptr, c_params, cvp);
    const data = STDVector.data(cvp);
    const len = STDVector.len(cvp);
    var buf = try std.ArrayList(u8).initCapacity(allocator, len);
    {
...
test "imgcodecs imdecode jpg" {
    ...
    const img_path = try std.fs.cwd().realpathAlloc(alloc, img_filename);
    defer alloc.free(img_path);

    const img_file = try std.fs.openFileAbsolute(img_path, .{});
    defer img_file.close();

    const stat = try img_file.stat();

    var buffer = try alloc.allocSentinel(u8, stat.size, 0);
    defer alloc.destroy(&buffer);

    var reader = img_file.reader(buffer);
    try reader.interface.readSliceAll(buffer);

    var img = try imDecode(buffer, .color);
    defer img.deinit();
