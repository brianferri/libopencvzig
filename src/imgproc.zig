...
///
pub fn cvtColor(src: Mat, dst: *Mat, code: ColorConversionCode) c.OpenCVResult {
    return c.CvtColor(src.ptr, dst.*.ptr, @intFromEnum(code));
}
...
///
pub fn gaussianBlur(src: Mat, dst: *Mat, ps: Size, sigma_x: f64, sigma_y: f64, border_type: BorderType) c.OpenCVResult {
    return c.GaussianBlur(src.ptr, dst.*.ptr, ps.toC(), sigma_x, sigma_y, border_type.toNum());
}
...
///
pub fn laplacian(src: Mat, dst: *Mat, d_depth: MatType, k_size: i32, scale: f64, delta: f64, border_type: BorderType) c.OpenCVResult {
...
///
pub fn scharr(src: Mat, dst: *Mat, d_depth: MatType, dx: i32, dy: i32, scale: f64, delta: f64, border_type: BorderType) c.OpenCVResult {
...
pub fn morphologyEx(src: Mat, dst: *Mat, op: MorphType, kernel: Mat) c.OpenCVResult {
    return c.MorphologyEx(src.ptr, dst.*.ptr, @intFromEnum(op), kernel.ptr);
}
...
/// For further details, please see:
/// https://docs.opencv.org/master/d4/d86/group__imgproc__filter.html#ga67493776e3ad1a3df63883829375201f
pub fn morphologyExWithParams(src: Mat, dst: *Mat, op: MorphType, kernel: Mat, iterations: i32, border_type: BorderType) c.OpenCVResult {
    const c_pt = Point.init(-1, -1).toC();
    return c.MorphologyExWithParams(src.ptr, dst.*.ptr, @intFromEnum(op), kernel.ptr, c_pt, iterations, border_type.toNum());
}
...
pub fn medianBlur(src: Mat, dst: *Mat, ksize: i32) c.OpenCVResult {
    return c.MedianBlur(src.ptr, dst.*.ptr, ksize);
}
...
pub fn canny(src: Mat, edges: *Mat, t1: f64, t2: f64) c.OpenCVResult {
    return c.Canny(src.ptr, edges.*.ptr, t1, t2);
}
...
///
pub fn cornerSubPix(img: Mat, corners: *Mat, winSize: Size, zeroZone: Size, criteria: TermCriteria) c.OpenCVResult {
    return c.CornerSubPix(img.ptr, corners.*.ptr, winSize.toC(), zeroZone.toC(), criteria.toC());
}
...
///
pub fn goodFeaturesToTrack(img: Mat, corners: *Mat, maxCorners: i32, quality: f64, minDist: f64) c.OpenCVResult {
    return c.GoodFeaturesToTrack(img.ptr, corners.*.ptr, maxCorners, quality, minDist);
}
...
pub fn grabCut(img: Mat, mask: *Mat, rect: Rect, bgd_model: *Mat, fgd_model: *Mat, iter_count: i32, mode: GrabCutMode) c.OpenCVResult {
    return c.GrabCut(img.ptr, mask.*.ptr, rect.toC(), bgd_model.*.ptr, fgd_model.*.ptr, iter_count, @intFromEnum(mode));
}
...
pub fn houghCircles(src: Mat, circles: *Mat, method: HoughMode, dp: f64, min_dist: f64) c.OpenCVResult {
    return c.HoughCircles(src.ptr, circles.*.ptr, @intFromEnum(method), dp, min_dist);
}
...
/// For further details, please see:
/// https://docs.opencv.org/master/dd/d1a/group__imgproc__feature.html#ga47849c3be0d0406ad3ca45db65a25d2d
pub fn houghCirclesWithParams(src: Mat, circles: *Mat, method: HoughMode, dp: f64, min_dist: f64, param1: f64, param2: f64, min_radius: i32, max_radius: i32) c.OpenCVResult {
    return c.HoughCirclesWithParams(src.ptr, circles.*.ptr, @intFromEnum(method), dp, min_dist, param1, param2, min_radius, max_radius);
}
...
pub fn houghLines(src: Mat, lines: *Mat, rho: f64, theta: f64, threshold_int: i32) c.OpenCVResult {
    return c.HoughLines(src.ptr, lines.*.ptr, rho, theta, threshold_int);
}
...
pub fn houghLinesP(src: Mat, lines: *Mat, rho: f64, theta: f64, threshold_int: i32) c.OpenCVResult {
    return c.HoughLinesP(src.ptr, lines.*.ptr, rho, theta, threshold_int);
}

pub fn houghLinesPWithParams(src: Mat, lines: *Mat, rho: f64, theta: f64, threshold_int: i32, minLineLength: f64, maxLineGap: f64) c.OpenCVResult {
    return c.HoughLinesPWithParams(src.ptr, lines.*.ptr, rho, theta, threshold_int, minLineLength, maxLineGap);
}
...
///
pub fn integral(src: Mat, sum: *Mat, sqsum: *Mat, tilted: *Mat) c.OpenCVResult {
    return c.Integral(src.ptr, sum.*.ptr, sqsum.*.ptr, tilted.*.ptr);
}
...
pub fn adaptiveThreshold(src: Mat, dst: *Mat, max_value: f64, adaptive_type: AdaptiveThresholdType, type_: ThresholdType, block_size: i32, C: f64) c.OpenCVResult {
    return c.AdaptiveThreshold(src.ptr, dst.*.ptr, max_value, @intFromEnum(adaptive_type), type_.toNum(), block_size, C);
}
...
//
pub fn arrowedLine(img: *Mat, pt1: Point, pt2: Point, color: Color, thickness: i32) c.OpenCVResult {
    return c.ArrowedLine(img.*.ptr, pt1.toC(), pt2.toC(), color.toScalar().toC(), thickness);
}
...
///
pub fn circle(img: *Mat, center: Point, radius: i32, color: Color, thickness: i32) c.OpenCVResult {
    return c.Circle(img.*.ptr, center.toC(), radius, color.toScalar().toC(), thickness);
}
...
/// https://docs.opencv.org/master/d6/d6e/group__imgproc__draw.html#gaf10604b069374903dbd0f0488cb43670
///
pub fn circleWithParams(img: *Mat, center: Point, radius: i32, color: Color, thickness: i32, line_type: LineType, shift: i32) c.OpenCVResult {
    return c.CircleWithParams(img.*.ptr, center.toC(), radius, color.toScalar().toC(), thickness, @intFromEnum(line_type), shift);
}
...
///
pub fn ellipse(img: *Mat, center: Point, axes: Point, angle: f64, start_angle: f64, end_angle: f64, color: Color, thickness: i32) c.OpenCVResult {
    return c.Ellipse(img.*.ptr, center.toC(), axes.toC(), angle, start_angle, end_angle, color.toScalar().toC(), thickness);
}
...
/// For further details, please see:
/// https://docs.opencv.org/master/d6/d6e/group__imgproc__draw.html#ga28b2267d35786f5f890ca167236cbc69
pub fn ellipseWithParams(img: *Mat, center: Point, axes: Point, angle: f64, start_angle: f64, end_angle: f64, color: Color, thickness: i32, line_type: LineType, shift: i32) c.OpenCVResult {
    return c.EllipseWithParams(img.*.ptr, center.toC(), axes.toC(), angle, start_angle, end_angle, color.toScalar().toC(), thickness, @intFromEnum(line_type), shift);
}
...
pub fn line(img: *Mat, pt1: Point, pt2: Point, color: Color, thickness: i32) c.OpenCVResult {
    return c.Line(img.ptr, pt1.toC(), pt2.toC(), color.toScalar().toC(), thickness);
}
...
///
pub fn rectangle(img: *Mat, rect: Rect, color: Color, thickness: i32) c.OpenCVResult {
    return c.Rectangle(img.*.ptr, rect.toC(), color.toScalar().toC(), thickness);
}
...
/// http://docs.opencv.org/master/d6/d6e/group__imgproc__draw.html#ga346ac30b5c74e9b5137576c9ee9e0e8c
///
pub fn rectangleWithParams(img: *Mat, rect: Rect, color: Color, thickness: i32, line_type: LineType, shift: i32) c.OpenCVResult {
    return c.RectangleWithParams(img.*.ptr, rect.toC(), color.toScalar().toC(), thickness, @intFromEnum(line_type), shift);
}
...
pub fn fillPoly(img: *Mat, points: PointsVector, color: Color) c.OpenCVResult {
    return c.FillPoly(img.*.ptr, points.toC(), color.toScalar().toC());
}
...
/// For more information, see:
/// https://docs.opencv.org/master/d6/d6e/group__imgproc__draw.html#gaf30888828337aa4c6b56782b5dfbd4b7
pub fn fillPolyWithParams(img: *Mat, points: PointsVector, color: Color, line_type: LineType, shift: i32, offset: Point) c.OpenCVResult {
    return c.FillPolyWithParams(img.*.ptr, points.toC(), color.toScalar().toC(), @intFromEnum(line_type), shift, offset.toC());
}
...
pub fn polylines(img: *Mat, points: PointsVector, is_closed: bool, color: Color, thickness: i32) c.OpenCVResult {
    return c.Polylines(img.*.ptr, points.toC(), is_closed, color.toScalar().toC(), thickness);
}
...
//
pub fn putText(img: *Mat, text: []const u8, org: Point, font_face: HersheyFont, font_scale: f64, color: Color, thickness: i32) c.OpenCVResult {
    return c.PutText(img.*.ptr, @as([*]const u8, @ptrCast(text)), org.toC(), font_face.toNum(), font_scale, color.toScalar().toC(), thickness);
}
...
// http://docs.opencv.org/master/d6/d6e/group__imgproc__draw.html#ga5126f47f883d730f633d74f07456c576
//
pub fn putTextWithParams(img: *Mat, text: []const u8, org: Point, font_face: HersheyFont, font_scale: f64, color: Color, thickness: i32, line_type: LineType, bottom_left_origin: bool) c.OpenCVResult {
    return c.PutTextWithParams(img.*.ptr, @as([*]const u8, @ptrCast(text)), org.toC(), font_face.toNum(), font_scale, color.toScalar().toC(), thickness, @intFromEnum(line_type), bottom_left_origin);
}
...
pub fn resize(src: Mat, dst: *Mat, sz: Size, fx: f64, fy: f64, interp: InterpolationFlag) c.OpenCVResult {
    return c.Resize(src.ptr, dst.*.ptr, sz.toC(), fx, fy, interp.toNum());
}
...
pub fn getRectSubPix(src: Mat, patch_size: Size, center: Point, dst: *Mat) c.OpenCVResult {
    return c.GetRectSubPix(src.ptr, patch_size.toC(), center.toC(), dst.*.ptr);
}
...
pub fn warpAffine(src: Mat, dst: *Mat, rot_mat: Mat, dsize: Size) c.OpenCVResult {
    return c.WarpAffine(src.ptr, dst.*.ptr, rot_mat.ptr, dsize.toC());
}
...
/// For further details, please see:
/// https://docs.opencv.org/master/da/d54/group__imgproc__transform.html#ga0203d9ee5fcd28d40dbc4a1ea4451983
pub fn warpAffineWithParams(src: Mat, dst: *Mat, rot_mat: Mat, dsize: Size, flags: InterpolationFlag, border_mode: BorderType, border_value: Color) c.OpenCVResult {
    return c.WarpAffineWithParams(src.ptr, dst.*.ptr, rot_mat.ptr, dsize.toC(), flags.toNum(), border_mode.toNum(), border_value.toScalar().toC());
}
...
pub fn warpPerspective(src: Mat, dst: *Mat, m: Mat, dsize: Size) c.OpenCVResult {
    return c.WarpPerspective(src.ptr, dst.*.ptr, m.ptr, dsize.toC());
}
...
/// For further details, please see:
/// https://docs.opencv.org/master/da/d54/group__imgproc__transform.html#gaf73673a7e8e18ec6963e3774e6a94b87
pub fn warpPerspectiveWithParams(src: Mat, dst: *Mat, rot_mat: Mat, dsize: Size, flags: InterpolationFlag, border_mode: BorderType, border_value: Color) c.OpenCVResult {
    return c.WarpPerspectiveWithParams(src.ptr, dst.*.ptr, rot_mat.ptr, dsize.toC(), flags.toNum(), border_mode.toNum(), border_value.toScalar().toC());
}
...
pub fn watershed(image: Mat, markers: *Mat) c.OpenCVResult {
    return c.Watershed(image.ptr, markers.*.ptr);
}
...
pub fn applyColorMap(src: Mat, dst: *Mat, colormap: ColormapType) c.OpenCVResult {
    return c.ApplyColorMap(src.ptr, dst.*.ptr, @intFromEnum(colormap));
}
...
pub fn applyCustomColorMap(src: Mat, dst: *Mat, colormap: Mat) c.OpenCVResult {
    return c.ApplyCustomColorMap(src.ptr, dst.*.ptr, colormap.ptr);
}
...
pub fn drawContours(src: *Mat, contours: PointsVector, contour_idx: i32, color: Color, thickness: i32) c.OpenCVResult {
    return c.DrawContours(src.*.ptr, contours.toC(), contour_idx, color.toScalar().toC(), thickness);
}
...
pub fn drawContoursWithParams(
    ...
    return c.DrawContoursWithParams(
        src.*.ptr,
        contours.toC(),
        contour_idx,
        color.toScalar().toC(),
...
///
pub fn sobel(src: Mat, dst: *Mat, ddepth: MatType, dx: i32, dy: i32, ksize: i32, scale: f64, delta: f64, border_type: BorderType) c.OpenCVResult {
    return c.Sobel(src.ptr, dst.*.ptr, @intFromEnum(ddepth), dx, dy, ksize, scale, delta, border_type.toNum());
}
...
///
pub fn spatialGradient(src: Mat, dx: *Mat, dy: *Mat, ksize: MatType, border_type: BorderType) c.OpenCVResult {
    return c.SpatialGradient(src.ptr, dx.*.ptr, dy.*.ptr, @intFromEnum(ksize), border_type.toNum());
}
...
pub fn remap(src: Mat, dst: *Mat, map1: Mat, map2: Mat, interpolation: InterpolationFlag, border_mode: BorderType, border_value: Color) c.OpenCVResult {
    return c.Remap(src.ptr, dst.*.ptr, map1.ptr, map2.ptr, interpolation.toNum(), border_mode.toNum(), border_value.toScalar().toC());
}
...
pub fn filter2D(src: Mat, dst: *Mat, ddepth: i32, kernel: Mat, anchor: Point, delta: f64, border_type: BorderType) c.OpenCVResult {
    return c.Filter2D(src.ptr, dst.*.ptr, ddepth, kernel.ptr, anchor.toC(), delta, border_type.toNum());
}
...
pub fn sepFilter2D(src: Mat, dst: *Mat, ddepth: i32, kernel_x: Mat, kernel_y: Mat, anchor: Point, delta: f64, border_type: BorderType) c.OpenCVResult {
    return c.SepFilter2D(src.ptr, dst.*.ptr, ddepth, kernel_x.ptr, kernel_y.ptr, anchor.toC(), delta, border_type.toNum());
}
...
pub fn logPolar(src: Mat, dst: *Mat, center: Point, m: f64, flags: InterpolationFlag) c.OpenCVResult {
    return c.LogPolar(src.ptr, dst.*.ptr, center.toC(), m, flags.toNum());
}
...
pub fn fitLine(pts: PointVector, line_mat: *Mat, dist_type: DistanceType, param: f64, reps: f64, aeps: f64) c.OpenCVResult {
    return c.FitLine(pts.toC(), line_mat.*.ptr, @intFromEnum(dist_type), param, reps, aeps);
}
...
///
pub fn linearPolar(src: Mat, dst: *Mat, center: Point, max_radius: f64, flags: InterpolationFlag) c.OpenCVResult {
    return c.LinearPolar(src.ptr, dst.*.ptr, center.toC(), max_radius, flags.toNum());
}
...
pub fn invertAffineTransform(src: Mat, dst: *Mat) c.OpenCVResult {
    return c.InvertAffineTransform(src.ptr, dst.*.ptr);
}
...
///
pub fn accumulate(src: Mat, dst: *Mat) c.OpenCVResult {
    return c.Mat_Accumulate(src.ptr, dst.*.ptr);
}
...
/// https://docs.opencv.org/master/d7/df3/group__imgproc__motion.html#ga1a567a79901513811ff3b9976923b199
///
pub fn accumulateWithMask(src: Mat, dst: *Mat, mask: Mat) c.OpenCVResult {
    return c.Mat_AccumulateWithMask(src.ptr, dst.*.ptr, mask.ptr);
}
...
///
pub fn accumulateSquare(src: Mat, dst: *Mat) c.OpenCVResult {
    return c.Mat_AccumulateSquare(src.ptr, dst.*.ptr);
}
...
/// https://docs.opencv.org/master/d7/df3/group__imgproc__motion.html#gacb75e7ffb573227088cef9ceaf80be8c
///
pub fn accumulateSquareWithMask(src: Mat, dst: *Mat, mask: Mat) c.OpenCVResult {
    return c.Mat_AccumulateSquareWithMask(src.ptr, dst.*.ptr, mask.ptr);
}
...
///
pub fn accumulateProduct(src1: Mat, src2: Mat, dst: *Mat) c.OpenCVResult {
    return c.Mat_AccumulateProduct(src1.ptr, src2.ptr, dst.*.ptr);
}
...
/// https://docs.opencv.org/master/d7/df3/group__imgproc__motion.html#ga82518a940ecfda49460f66117ac82520
///
pub fn accumulateProductWithMask(src1: Mat, src2: Mat, dst: *Mat, mask: Mat) c.OpenCVResult {
    return c.Mat_AccumulateProductWithMask(src1.ptr, src2.ptr, dst.*.ptr, mask.ptr);
}
...
///
pub fn accumulatedWeighted(src: Mat, dst: *Mat, alpha: f64) c.OpenCVResult {
    return c.Mat_AccumulatedWeighted(src.ptr, dst.*.ptr, alpha);
}
...
/// https://docs.opencv.org/master/d7/df3/group__imgproc__motion.html#ga4f9552b541187f61f6818e8d2d826bc7
///
pub fn accumulatedWeightedWithMask(src: Mat, dst: *Mat, alpha: f64, mask: Mat) c.OpenCVResult {
    return c.Mat_AccumulatedWeightedWithMask(src.ptr, dst.*.ptr, alpha, mask.ptr);
}
