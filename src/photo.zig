...
///
pub fn colorChange(src: Mat, mask: Mat, dst: *Mat, red_mul: f32, green_mul: f32, blue_mul: f32) c.OpenCVResult {
    return c.ColorChange(src.ptr, mask.ptr, dst.*.ptr, red_mul, green_mul, blue_mul);
}
...
///
pub fn seamlessClone(src: Mat, dst: *Mat, mask: Mat, p: Point, blend: Mat, flags: SeamlessCloneFlag) c.OpenCVResult {
    return c.SeamlessClone(src.ptr, dst.*.ptr, mask.ptr, p.toC(), blend.ptr, @intFromEnum(flags));
}
...
///
pub fn illuminationChange(src: Mat, mask: Mat, dst: *Mat, alpha: f32, beta: f32) c.OpenCVResult {
    return c.IlluminationChange(src.ptr, mask.ptr, dst.*.ptr, alpha, beta);
}
...
///
pub fn textureFlattening(src: Mat, mask: Mat, dst: *Mat, low_threshold: f32, high_threshold: f32, kernel_size: i32) c.OpenCVResult {
    return c.TextureFlattening(src.ptr, mask.ptr, dst.*.ptr, low_threshold, high_threshold, kernel_size);
}
...
pub fn fastNlMeansDenoisingColoredMulti(
    ...
    return c.FastNlMeansDenoisingColoredMulti(c_mats, dst.*.ptr, img_to_denoise_index, temporal_window_size);
}
...
pub fn fastNlMeansDenoisingColoredMultiWithParams(
    ...
    const c_mats = try Mat.toCStructs(src);
    defer Mat.deinitCStructs(c_mats);
    return c.FastNlMeansDenoisingColoredMultiWithParams(
        c_mats,
        dst.*.ptr,
...
///
pub fn fastNlMeansDenoising(src: Mat, dst: *Mat) c.OpenCVResult {
    return c.FastNlMeansDenoising(src.ptr, dst.*.ptr);
}
...
pub fn fastNlMeansDenoisingWithParams(
    ...
    return c.FastNlMeansDenoisingWithParams(
        src.ptr,
        dst.*.ptr,
        h,
        template_window_size,
...
///
pub fn fastNlMeansDenoisingColored(src: Mat, dst: *Mat) c.OpenCVResult {
    return c.FastNlMeansDenoisingColored(src.ptr, dst.*.ptr);
}
...
pub fn fastNlMeansDenoisingColoredWithParams(
    ...
    return c.FastNlMeansDenoisingColoredWithParams(
        src.ptr,
        dst.*.ptr,
        h,
        h_color,
...
///
pub fn detailEnhance(src: Mat, dst: *Mat, sigma_s: f32, sigma_r: f32) c.OpenCVResult {
    return c.DetailEnhance(src.ptr, dst.*.ptr, sigma_s, sigma_r);
}
...
///
pub fn edgePreservingFilter(src: Mat, dst: *Mat, flags: EdgeFilter, sigma_s: f32, sigma_r: f32) c.OpenCVResult {
    return c.EdgePreservingFilter(src.ptr, dst.*.ptr, @intFromEnum(flags), sigma_s, sigma_r);
}
...
/// https://docs.opencv.org/4.x/df/dac/group__photo__render.html#gafaee2977597029bc8e35da6e67bd31f7
///
pub fn edgePreservingFilterWithKernel(src: Mat, dst: *Mat, kernel: Mat) c.OpenCVResult {
    return c.EdgePreservingFilterWithKernel(src.ptr, dst.*.ptr, kernel.ptr);
}
...
/// https://docs.opencv.org/4.x/df/dac/group__photo__render.html#gae5930dd822c713b36f8529b21ddebd0c
///
pub fn pencilSketch(src: Mat, dst1: *Mat, dst2: *Mat, sigma_s: f32, sigma_r: f32, shade_factor: f32) c.OpenCVResult {
    return c.PencilSketch(src.ptr, dst1.*.ptr, dst2.*.ptr, sigma_s, sigma_r, shade_factor);
}
...
///
pub fn stylization(src: Mat, dst: *Mat, sigma_s: f32, sigma_r: f32) c.OpenCVResult {
    return c.Stylization(src.ptr, dst.*.ptr, sigma_s, sigma_r);
}
...
test "photo colorchange" {
    ...
    _ = colorChange(src, mask, &dst, 1.5, 1.5, 1.5);
...
test "photo seamlessClone" {
    ...
    _ = seamlessClone(src, &dst, mask, center, blend, .normal_clone);
...
test "photo illumination change" {
    ...
    _ = illuminationChange(src, mask, &dst, 0.2, 0.4);
...
test "photo textureFlattening" {
    ...
    _ = textureFlattening(src, mask, &dst, 30, 45, 3);
...
test "photo fastNlMeansDenoisingColoredMulti" {
    ...
    _ = try fastNlMeansDenoisingColoredMulti(src[0..], &dst, 1, 1);
...
test "photo fastNlMeansDenoisingColoredMultiWithParams" {
    ...
    _ = try fastNlMeansDenoisingColoredMultiWithParams(src[0..], &dst, 1, 1, 3, 3, 7, 21);
...
test "photo fastNlMeansDenoising" {
    ...
    _ = fastNlMeansDenoising(img, &dst);
...
test "photo FastNlMeansDenoisingColoredMultiWithParams" {
    ...
    _ = fastNlMeansDenoisingWithParams(img, &dst, 3, 7, 21);
...
test "photo fastNlMeansDenoisingColored" {
    ...
    _ = fastNlMeansDenoisingColored(img, &dst);
...
test "photo fastNlMeansDenoisingColoredWithParams" {
    ...
    _ = fastNlMeansDenoisingColoredWithParams(img, &dst, 3, 3, 7, 21);
...
test "photo detailEnhance" {
    ...
    _ = detailEnhance(src, &dst, 100, 0.5);
...
test "photo edgePreservingFilter" {
    ...
    _ = edgePreservingFilter(src, &dst, .recurs_filter, 100, 0.5);
...
test "photo pencilSketch" {
    ...
    _ = pencilSketch(src, &dst1, &dst2, 100, 0.5, 0.5);
