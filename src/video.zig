...
test "video CalcOpticalFlow.pyrLK" {
    ...
    _ = imgproc.cvtColor(img1, &dst, .bgra_to_gray);
...
test "video CalcOpticalFlow.pyrLK with params" {
    ...
    _ = imgproc.goodFeaturesToTrack(dst, &corners, 500, 0.01, 10);
    var tc = try core.TermCriteria.init(.{ .count = true, .eps = true }, 20, 0.03);
    defer tc.deinit();
    _ = imgproc.cornerSubPix(
        dst,
        &corners,
        Size.init(10, 10),
        Size.init(-1, -1),
...
test "video findTransformECC" {
    ...
    _ = imgproc.resize(img1, &test_img, Size.init(216, 216), 0, 0, .{ .type = .linear });
    ...
    _ = imgproc.warpAffineWithParams(
        test_img,
        &warped_img,
        translation_ground,
        Size.init(200, 200),
